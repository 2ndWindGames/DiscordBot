...
var industryConfig IndustryConfig
...
func runIndustryBot(config IndustryConfig) {
	industryConfig = config
	log.Println("🎮 Korea Game Industry Bot started")
	log.Printf("📰 feeds: %d", len(feeds))
	log.Printf("⏱️ interval: %s", industryConfig.CheckInterval())
	...
	ticker := time.NewTicker(industryConfig.CheckInterval())
	defer ticker.Stop()
...
func initializeFeeds() {
	...
	*/
	...
	limit := industryConfig.InitialArticleCount
...
func checkFeed(feed Feed) {
	...
	*/
	...
	/*
		...
		Discord 도배 방지
	*/
	if len(newArticles) > industryConfig.MaxArticlesPerCheck {
		newArticles =
			newArticles[len(newArticles)-industryConfig.MaxArticlesPerCheck:]
	}
...
func sendArticle(article Article) error {
	...
	*/
	...
	return postDiscordToAll(industryConfig.WebhookURLs(), payload)
}
...
func sendSystemMessage(
	...
	if err := postDiscordToAll(industryConfig.WebhookURLs(), payload); err != nil {
		log.Printf(
